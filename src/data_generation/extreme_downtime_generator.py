"""
Extreme downtime simulation generator for clearly visible connector outages.
This generator creates guaranteed downtime periods that will be dramatically visible in charts.
"""
...
class ExtremeDowntimeTransactionGenerator:
    """Generator with guaranteed dramatic downtime periods."""
    
    def __init__(self, seed: Optional[int] = None):
        """Initialize the extreme downtime transaction generator."""
        if seed:
            random.seed(seed)
            np.random.seed(seed)
        ...
        # Create guaranteed downtime periods for each connector
        self.downtime_periods = self._create_guaranteed_downtime_periods()
    ...
    def _create_guaranteed_downtime_periods(self) -> Dict[str, List[Dict]]:
        """Create guaranteed downtime periods that will be clearly visible."""
        downtime_periods = {}
        ...
        for connector in self.connectors:
            ...
            # Create multiple guaranteed downtime periods over the last 7 days
            for day_offset in range(7):
                day_start = now - timedelta(days=day_offset)
                
                # Major outage: 4-6 hour complete failure (once per connector over 7 days)
                if day_offset == random.randint(0, 6):
                    outage_start = day_start.replace(hour=random.randint(8, 14), minute=0, second=0, microsecond=0)
                    outage_duration = random.randint(4, 6)
                    periods.append({
                        "type": "major_outage",
                        "start": outage_start,
                        "end": outage_start + timedelta(hours=outage_duration),
                        "failure_rate": 0.98,  # 98% failure rate
                        "reason": "system_outage"
                    })
                
                # Maintenance window: 2-3 hour high failure rate (every 2-3 days)
                if day_offset % 2 == 0:
                    maintenance_start = day_start.replace(hour=random.randint(2, 5), minute=0, second=0, microsecond=0)
                    maintenance_duration = random.randint(2, 3)
                    periods.append({
                        "type": "maintenance",
                        "start": maintenance_start,
                        "end": maintenance_start + timedelta(hours=maintenance_duration),
                        "failure_rate": 0.85,  # 85% failure rate
                        "reason": "scheduled_maintenance"
                    })
                
                # Peak hour degradation: 1-2 hour moderate failure (daily during business hours)
                peak_start = day_start.replace(hour=random.randint(10, 15), minute=0, second=0, microsecond=0)
                peak_duration = random.randint(1, 2)
                periods.append({
                    "type": "peak_degradation",
                    "start": peak_start,
                    "end": peak_start + timedelta(hours=peak_duration),
                    "failure_rate": 0.65,  # 65% failure rate
                    "reason": "peak_hour_overload"
                })
            
            downtime_periods[connector.id] = periods
        
        return downtime_periods
    
    def _is_connector_in_downtime(self, connector_id: str, timestamp: datetime) -> Tuple[bool, Optional[str], float]:
        """Check if connector is in a guaranteed downtime period."""
        if connector_id not in self.downtime_periods:
            return False, None, 0.0
        
        for period in self.downtime_periods[connector_id]:
            if period["start"] <= timestamp <= period["end"]:
                return True, period["reason"], period["failure_rate"]
    ...
    def _determine_transaction_status_with_extreme_downtime(self, connector_id: str, risk_score: float, 
                                                          amount: float, timestamp: datetime,
                                                          payment_method: PaymentMethod) -> Tuple[TransactionStatus, Optional[str], float]:
        """Determine transaction status with extreme downtime simulation."""
        
        # Check for guaranteed downtime first
        is_down, downtime_reason, failure_rate = self._is_connector_in_downtime(connector_id, timestamp)
        if is_down:
            # During downtime, apply the guaranteed failure rate
            if random.random() < failure_rate:
                if downtime_reason == "system_outage":
                    return TransactionStatus.FAILED, "connector_unavailable", 0.0
                elif downtime_reason == "scheduled_maintenance":
        ...
        # Normal processing if not in downtime or if transaction passes downtime check
    ...
    def generate_time_range_dataset(self, merchant_id: str, time_range: TimeRange, 
        ...
        """Generate dataset for specified time range with extreme downtime."""
        ...
        print(f"Generating {total_transactions} transactions with EXTREME DOWNTIME from {start_time} to {end_time}")
        
        # Print downtime periods for visibility
        print("\n🚨 GUARANTEED DOWNTIME PERIODS:")
        for connector_id, periods in self.downtime_periods.items():
            for period in periods:
                if period["start"] >= start_time and period["end"] <= end_time:
                    print(f"  {connector_id}: {period['type']} from {period['start']} to {period['end']} ({period['failure_rate']*100:.0f}% failure rate)")
        print()
        ...
        for i in range(total_transactions):
            ...
            transaction = self.generate_extreme_downtime_transaction(merchant_id, transaction_time)
        ...
        print(f"Final dataset: {len(transactions)} transactions with EXTREME DOWNTIME SIMULATION")
        return transactions
    
    def generate_extreme_downtime_transaction(self, merchant_id: str, 
                                            timestamp: Optional[datetime] = None) -> Transaction:
        """Generate a single transaction with extreme downtime simulation."""
        ...
        # Determine transaction status with extreme downtime logic
        status, failure_reason, cost = self._determine_transaction_status_with_extreme_downtime(
            connector.id, risk_score, amount, timestamp, payment_method
        )
        
        # Calculate realistic processing time
        is_downtime = failure_reason in ["scheduled_maintenance", "connector_unavailable", "rate_limit_exceeded"]
        ...
        if status in [TransactionStatus.TIMEOUT, TransactionStatus.FAILED]:
            if failure_reason in ["network_timeout", "connector_unavailable", "rate_limit_exceeded"]:
    ...
    def save_to_csv(self, transactions: List[Transaction], filename: str):
        ...
        print(f"Saved {len(transactions)} EXTREME DOWNTIME transactions to {filename}")
    ...
    def generate_analysis_report(self, transactions: List[Transaction]) -> Dict:
        ...
        for transaction in transactions:
            if transaction.connector_id:
                ...
                if transaction.failure_reason in ["scheduled_maintenance", "connector_unavailable", "rate_limit_exceeded"]:
        ...
        return {
            ...
            'downtime_summary': self._analyze_downtime_impact(transactions)
        }
    
    def _analyze_downtime_impact(self, transactions: List[Transaction]) -> Dict:
        """Analyze the impact of downtime periods."""
        downtime_reasons = ["scheduled_maintenance", "connector_unavailable", "rate_limit_exceeded"]
        downtime_transactions = [t for t in transactions if t.failure_reason in downtime_reasons]
        
        downtime_impact = {
            "total_downtime_transactions": len(downtime_transactions),
            "downtime_failure_rate": len(downtime_transactions) / len(transactions) if transactions else 0,
            "downtime_by_reason": {},
            "downtime_by_connector": {}
        }
        
        # Analyze by reason
        for reason in downtime_reasons:
            reason_transactions = [t for t in downtime_transactions if t.failure_reason == reason]
            downtime_impact["downtime_by_reason"][reason] = len(reason_transactions)
        
        # Analyze by connector
        for transaction in downtime_transactions:
            connector_id = transaction.connector_id
            if connector_id not in downtime_impact["downtime_by_connector"]:
                downtime_impact["downtime_by_connector"][connector_id] = 0
            downtime_impact["downtime_by_connector"][connector_id] += 1
        
        return downtime_impact
